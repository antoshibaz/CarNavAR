...
internal class ArLane(context: Context) {

    companion object {

        internal const val LANE_DEFAULT_WIDTH = 0.1f // meters
...
void main()
{
    ...
    gl_FragColor = vec4(finalColor.xyz, uColor.w * vTexCoords.y);
        ...
        fun getPerspectiveProjMatrix(camera: Camera): Matrix4 {
            // Get projection matrix.
            val projmtx = FloatArray(16)
            camera.getProjectionMatrix(projmtx, 0, 0.1f, 1000.0f)
            return Matrix4(projmtx)
        }

        fun getViewMatrix(camera: Camera): Matrix4 {
            // Get camera matrix.
            val viewmtx = FloatArray(16)
            camera.getViewMatrix(viewmtx, 0)
            return Matrix4(viewmtx)
        }

        fun getViewProjectionMatrix(camera: Camera): Matrix4 {
            return getPerspectiveProjMatrix(camera) * getViewMatrix(camera)
        }
